...
import { schedulePlaybackBinding } from './watched-history/playback-utils.js';
...
async function hydrateWatchedIdCache() {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onerror = () => reject(request.error || new Error('Failed to read watched IDs'));
    });
}
...
async function persistPendingCloudSyncIds(videoIds) {
    ...
    for (const rawId of videoIds || []) {
        ...
        if (!isValidVideoId(videoId) || seenIncoming.has(videoId)) continue;
    ...
    try {
        ...
        for (const rawId of existingRaw) {
            const videoId = typeof rawId === 'string' ? rawId.trim() : '';
            if (!isValidVideoId(videoId) || seen.has(videoId)) continue;
            seen.add(videoId);
            merged.push(videoId);
        }
    ...
    if (!runtimeMessageListener) {
        runtimeMessageListener = (message, _sender, sendResponse) => {
            ...
            if (message.type === 'ACK_SYNCED_VIDEO_IDS') {
                ackSyncedVideoIdsLocal(message.videoIds)
...
async function addToWatchedHistory(videoId) {
    ...
    if (!backgroundQueued) await persistPendingCloudSyncIds([videoId]);
...
async function ackSyncedVideoIdsLocal(videoIds) {
    await ensureInitialized();
    ...
    return ackSyncedVideoIds(unique);
}

async function seedSyncQueueFromHistory() {
    await ensureInitialized();
    return seedSyncQueue(watchedIds);
}
...
async function clearWatchedHistory() {
    ...
    const identity = getSyncAccountIdentityState();
    await clearSyncQueue(identity.accountKey);
}
...
async function importWatchedHistory(videoIds, options = {}) {
    await ensureInitialized();
    const db = getDb();
    if (!Array.isArray(videoIds) || videoIds.length === 0 || !db) return 0;
    ...
    for (const rawId of videoIds) {
        ...
        if (!isValidVideoId(trimmed) || watchedIds.has(trimmed) || seenInBatch.has(trimmed))
            continue;
        seenInBatch.add(trimmed);
        uniqueToAdd.push(trimmed);
    }
    if (uniqueToAdd.length === 0) return 0;
    ...
    await new Promise((resolve, reject) => {
        ...
        uniqueToAdd.forEach((videoId, index) => {
            ...
            if (queueStore) queueStore.put({ videoId, queuedAt: startTimestamp + index });
        });
        transaction.oncomplete = () => resolve();
        transaction.onerror = () =>
    ...
    if (!skipSyncQueue && !backgroundQueued) await persistPendingCloudSyncIds(uniqueToAdd);
...
function resetVisualDecorations() {
    ...
    document
        .querySelectorAll(`[${WATCHED_ATTR}='true']`)
        .forEach((thumb) => thumb.removeAttribute(WATCHED_ATTR));
    document
        .querySelectorAll(`.${HIDDEN_CLASS}`)
        .forEach((container) => container.classList.remove(HIDDEN_CLASS));
}
